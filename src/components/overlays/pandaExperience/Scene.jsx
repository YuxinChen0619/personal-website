...
import { stationAtProgress } from "./stations";
...
const Scene = ({
  ...
  const cameraScrollCurve = useScrollCurve(
    ...
    initialCameraPoints,
    SHIFT_X_AMOUNT,
    initialSceneSnapshot.loopCounter,
  );
  ...
  const [rotationBufferQuat] = useState(
    new THREE.Quaternion(...initialSceneSnapshot.cameraGroupQuaternion),
  );
  ...
  useFrame(() => {
    ...
    // 场景在循环衔接时会经过一段没有地面的空中过渡。
    // 这段时间不属于任何经历站点，不能沿用同一 progress 显示卡片。
    const station = cameraScrollCurve.transitionCurveActive.current
      ? null
      : stationAtProgress(newProgress);
    ...
    onSceneSnapshotChange?.({
      ...
      loopCounter: cameraScrollCurve.loopCounter.current,
      sceneOffsetX: sceneGroupRef.current?.position.x ?? initialSceneSnapshot.sceneOffsetX,
      singleSheetOffsetX: singleSheetRef.current?.position.x ?? initialSceneSnapshot.singleSheetOffsetX,
      activeStation: station,
      cameraGroupPosition: [cameraGroup.current.position.x, cameraGroup.current.position.y, cameraGroup.current.position.z],
      cameraGroupQuaternion: [cameraGroup.current.quaternion.x, cameraGroup.current.quaternion.y, cameraGroup.current.quaternion.z, cameraGroup.current.quaternion.w],
    });
  });
  ...
  return (
    <>
      <Suspense fallback={null}>
        <group ref={sceneGroupRef} position-x={initialSceneSnapshot.sceneOffsetX}>
        ...
        <Panda
          ...
          initialLoopCounter={initialSceneSnapshot.loopCounter}
        />
        <group ref={singleSheetRef} position-x={initialSceneSnapshot.singleSheetOffsetX}>
