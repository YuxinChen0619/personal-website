 ...
 *   `AUTOFOCUS_END`   状态机推进到 idle —— 参考里 6.40s 才有第一次点击
 *
 * 为什么中间有一段「不动」：4.76s 与 5.56s 两处的边缘检测
 * （柜位立柱、门缝、bay4 把手）逐条对比**全部落在 1px 以内**，
 * 整帧灰度平均绝对差只有 3.35，差异来自鼠标指针和贴纸微动。
 * 也就是说参考的推近 4.76s 就已经到位，之后是**停住等用户操作**，
 * 而不是「一直推到 6.40s」。这里照此实现：0.57s 推到位，然后 hold。
 */
export const AUTOFOCUS_START = 2420
export const AUTOFOCUS_ARRIVE = 2990
export const AUTOFOCUS_END = 4630
